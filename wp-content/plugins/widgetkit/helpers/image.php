	...
	public function prepareLazyload($content) {
		...
		if (stripos($content, '<img') !== false) {
            ...
            foreach ($images as $img) {
            	...
            	preg_match('/width=["|\']([^"]*)["|\']/i',$img, $width);
            	preg_match('/height=["|\']([^"]*)["|\']/i',$img, $height);

            	if ($width && $height) {
	            	if ($this->cWidth != $width[1] || $this->cHeight != $height[1]) {
	            		$this->cWidth = $width[1];
	            		$this->cHeight = $height[1];

	            		if ($image = imagecreatetruecolor($this->cWidth, $this->cHeight) ) {
							...
							$this->image_data_base64 = base64_encode ($image_data);

		            	}
	            	}

	            	$tmp_image = str_replace($src[0], 'src="data:image/png;base64,'.$this->image_data_base64.'" data-'.$src[0], $img);
	            	$content = str_replace($img, $tmp_image, $content);
            	}
            }
        }
